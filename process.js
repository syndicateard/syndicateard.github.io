...
function AddProduk() {
  ...
  newRow.getElementsByTagName("select")[0].name = `SatJumlahProduk-${newRowId}`;
  newRow.getElementsByTagName("span")[0].id = `total-${newRowId}`;
  newRow.getElementsByTagName("span")[0].innerHTML = "0";
  if (newRowId == 1) {
    $("#Produk-1").append(
      `<td><button class="remProduk" type="button">-</button></td>`
    );
  }
  newRow
    .getElementsByTagName("button")[0]
    .addEventListener("click", removeProduk);
}
...
function varToPrintContent() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const HargaForRow = parseInt(
      rows[i].getElementsByTagName("input")[1].value
    );
    let jumlahForRow = parseFloat(
      rows[i].getElementsByTagName("input")[2].value
    );
    jumlahForRow = jumlahForRow.toString().replace(".", ",");
    const satuanForRow = rows[i].getElementsByTagName("select")[0].value;
    const totalForRow =
      "Rp " +
      rupiah(parseInt(rows[i].getElementsByTagName("span")[0].innerHTML));

    $("#pid_produk").append(
      `<li>
            <div class="p_namaproduk">` +
        namaForRow +
        `</div>
  ...
  $("#pid_totalhargaproduk").html(
    "Rp " + rupiah($("#totalhargabarang").html())
  );
  $("#pid_ongkir").html("Rp " + rupiah($("#BiayaOngkir").val()));
  $("#pid_alamat").html(
    "<strong>Alamat Tujuan : </strong>" + $("#AlamatKirim").val()
  );
  $("#pid_totalitembawah").html(
    "Total - " + $("#totalbarang").html() + " Item"
  );
